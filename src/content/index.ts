...
function optimize(): void {
  ...
  document
    .querySelectorAll<HTMLElement>(
      ".bili-video-card,.feed-card,.video-page-card-small,.bili-live-card,[class*='video-card']",
    )
    .forEach((card) => {
      const text = textOf(card);
      const ad =
        /(^|\s)(广告|推广)(\s|$)/.test(text) ||
        Boolean(card.querySelector("[class*='ad-tag']"));
      const live =
        /(^|\s)直播(中)?(\s|$)/.test(text) ||
        Boolean(card.querySelector("[class*='live']"));
      const badge = settings.interfaceOptimization.hiddenBadges.some(
        (value) => value.trim() && text.includes(value.trim()),
      );
      if (
        ...
        card.dataset.bseCardHidden = "true";
      }
    });
}
